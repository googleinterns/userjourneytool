...
def test_write_proto_to_file(patch_path):
    ...
    with patch(f"{patch_path}.text_format.MessageToString"
              ) as mock_message_to_string:
...
def test_read_proto_from_file(patch_path):
    ...
    with patch(f"{patch_path}.text_format.Parse",
               Mock(return_value=sentinel.result)) as mock_parse:
...
def test_read_write_functional(patch_path, tmp_path):
    ...
    ujt.server.server_utils.write_proto_to_file(
        tmp_path /
        ujt.server.server_utils.named_proto_file_name(node.name,
                                                      Node),
        node,
    )
