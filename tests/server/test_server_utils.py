from unittest.mock import MagicMock, Mock, mock_open, patch, sentinel, call
...
def test_named_proto_file_name():
    ...
    assert ujt.server.server_utils.named_proto_file_name(
        sentinel.name,
        mock_proto_type) == "sentinel.type_sentinel.name.ujtdata"
...
def test_write_proto_to_file(patch_path):
    ...
    with patch(f"{patch_path}.text_format.MessageToString") as mock_message_to_string:
        ...
        mock_path.write_text.assert_called_once_with(mock_message_to_string.return_value)
...
def test_read_proto_from_file(patch_path):
    ...
    with patch(f"{patch_path}.text_format.Parse", Mock(return_value=sentinel.result)) as mock_parse:
        ujt.server.server_utils.read_proto_from_file(mock_path, mock_proto_type)

        assert mock_path.read_text.mock_calls == [call()]
        mock_parse.assert_called_once_with(mock_path.read_text.return_value, mock_proto_type.return_value)
