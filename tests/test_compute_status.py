...
from unittest.mock import sentinel, patch, Mock, call, ANY

from graph_structures_pb2 import Node, SLI, Status, Dependency, Client, UserJourney
...
@pytest.mark.parametrize("expected_status", [Status.STATUS_HEALTHY, Status.STATUS_WARN, Status.STATUS_ERROR])
def test_compute_statuses(patch_path, expected_status):
        ...
        client_name: Client(
            ...
            user_journeys=[
                UserJourney(
                    ...
                    dependencies=[
                        Dependency(source_name=f"{client_name}.{user_journey_name}", target_name=node_name)
                    ],
                ),
            ],
        ),
    ...
    with patch(
            ...
            Mock(
                side_effect=side_effect_update_node_name_message_map
            )
        ) as mock_compute_single_node_status, \
        patch(f"{patch_path}.compute_status_from_count_map", Mock(return_value=expected_status)) as mock_compute_status_from_count_map:
        ujt.compute_status.compute_statuses(node_name_message_map, client_name_message_map)

        assert client_name_message_map[client_name].user_journeys[0].status == expected_status
        ...
        assert mock_compute_single_node_status.mock_calls == [call(ANY, node_name), call(ANY, orphan_name)]
        assert mock_compute_status_from_count_map.mock_calls == [call({expected_status: 1})]
...
def test_compute_single_node_status_already_set():
    ...
    assert ujt.compute_status.compute_single_node_status(node_name_message_map, node_name) == Status.STATUS_HEALTHY
...
def test_compute_single_node_status_with_child(expected_status):
    ...
    assert ujt.compute_status.compute_single_node_status(node_name_message_map, parent_name) == expected_status
...
def test_compute_single_node_status_with_dependency(expected_status):
    ...
    assert ujt.compute_status.compute_single_node_status(node_name_message_map, source_name) == expected_status
    assert node_name_message_map[source_name].status == expected_status

@pytest.mark.parametrize(
    "sli_value, expected_status",
    [
        (.5, Status.STATUS_HEALTHY),
        (.15, Status.STATUS_WARN),
        (.85, Status.STATUS_WARN),
...
def test_compute_single_node_status_with_sli(sli_value, expected_status, slo_bounds):
    ...
    node_name_message_map = {
        node_name: Node(
            name=node_name, 
            slis=[
                SLI(node_name=node_name, sli_value=sli_value, **slo_bounds)
            ])
    }
    assert ujt.compute_status.compute_single_node_status(node_name_message_map, node_name) == expected_status
    assert node_name_message_map[node_name].status == expected_status
...
def test_compute_status_from_count_map(count_map, expected_status):
    assert ujt.compute_status.compute_status_from_count_map(count_map) == expected_status
